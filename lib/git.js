...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield (0, execute_1.execute)(`rsync ${action.silent ? '-q' : ''} -av --checksum --progress ${action.folderPath}/. ${action.targetFolder
            ...
            yield (0, execute_1.execute)(`ls -al ${action.folderPath}`, action.workspace, action.silent);
            console.log('===================== targetFolder', action.targetFolder);
            yield (0, execute_1.execute)(`ls -al ${action.targetFolder
                ? `${temporaryDeploymentDirectory}/${action.targetFolder}`
                : temporaryDeploymentDirectory}`, action.workspace, action.silent);
            ...
            const hasFilesToCommit = action.isTest & constants_1.TestFlag.HAS_CHANGED_FILES ||
                Boolean((yield (0, execute_1.execute)(checkGitStatus, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent)).stdout);
